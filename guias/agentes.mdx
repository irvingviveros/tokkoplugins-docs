...
<Steps>
  <Step title="Verifica la opción">
    Confirma que **Enable Agent Sync** (`thi_enable_agent_sync`) esté activada en la sección de **Agents** del plugin.
  </Step>
  <Step title="Verifica la API Key">
    Ejecuta **Test Connection** en **Tokko Plugins → Configuration → API Connection**. Si la conexión falla, revisa la [guía de API Key](/guias/api-key) para regenerarla.
  </Step>
  <Step title="Confirma que haya productores en Tokko">
    En Tokko Broker, revisa que tus propiedades tengan un productor asignado. Sólo los agentes vinculados a propiedades sincronizadas aparecen en WordPress.
  </Step>
</Steps>
